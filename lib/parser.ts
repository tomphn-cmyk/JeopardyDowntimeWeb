...
const parseClueCoordinates = (clueId: string): { row: number; column: number } | null => {
  const match = clueId.match(/^clue_J_(\d+)_(\d+)$/);
  if (!match) {
    return null;
  }
...
const parseJArchiveCategory = (rawHtml: string): JeopardyCategory | null => {
  ...
  round.find('td.clue').each((_, clueElement) => {
    ...
    const clueId = clue.attr('id') || clue.find('[id^="clue_J_"]').first().attr('id') || '';
    const coordinates = parseClueCoordinates(clueId);
    ...
    console.debug('[parser] Clue coordinate parsed:', {
      clueId,
      row,
      column,
      categoryIndex,
  ...
  console.debug('[parser] Clue counts per column:', counts);
  ...
  console.debug('[parser] Final selected category:', {
    ...
    clueValues: clues.map((clue) => clue.value),
  });
